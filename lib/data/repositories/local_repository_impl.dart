...
class LocalRepositoryImpl implements LocalRepository {
  ...
  Future<Either<Failure, bool>> hasLogIn() async {
    ...
    result.fold((l) => {},
        (r) => {hasLogIn = (r.token != "" && r.getOrgaId() != null)});
  ...
  Future<Either<Failure, List<String>>> getSideMenuListOptions() async {
    ...
    if (roles.contains(Roles.roleAnonymous)) {
      ...
      if (roles.contains(Roles.roleSuperAdmin)) {
        opts.add(SideDrawerUserOptions.optWelcome);
  ...
  @override
  Future<Either<Failure, String>> getLocalValue(String key) async {
    try {
      return Right(await localDataSource.getLocalValue(key));
    } on CacheException {
      return Future.value(
          const Left(ConnectionFailure('No se pudo leer el valor local')));
    }
  }

  @override
  Future<Either<Failure, bool>> setLocalValue(String key, String value) async {
    try {
      return Right(await localDataSource.setLocalValue(key, value));
    } on CacheException {
      return Future.value(
          const Left(ConnectionFailure('No se pudo escribir el valor local')));
    }
  }
}
